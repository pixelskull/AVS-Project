...
//            print("viewDidChangeBackingProperties, new scale = \(window!.backingScaleFactor)")
        ...
        override func updateTrackingAreas() {
            ...
            trackingArea = NSTrackingArea(rect: trackingRect, options: [.ActiveAlways, .MouseEnteredAndExited, .MouseMoved], owner: self, userInfo: nil)
...
class LineChart: CALayer {
    ...
    override func layoutSublayers() {
        ...
        for yAxis in yAxes {
            ...
            let maxYValue = datasetsForAxis.reduce(CGFloat.min, combine: { max($0, $1.data.minElement()!) })
    ...
    var attributedString : NSAttributedString {
        if let attString = string as? NSAttributedString  {
            ...
            if layerFont == nil {
                ...
                layerFont = CTFontCreateUIFontForLanguage(.System, fontSize, nil)
            }
            //return NSAttributedString(string: string as NSString, attributes: [NSFontAttributeName: layerFont!])
            return NSAttributedString(string: string as String, attributes: [(kCTFontAttributeName as String): layerFont!])
        } else {
