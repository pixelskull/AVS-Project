...
public class LineChart: NSView {
    ...
    */
    private class Helpers {
        ...
        */
        private class func UIColorFromHex(hex: Int) -> NSColor {
        ...
        * Lighten color.
        */
        private class func lightenUIColor(color: NSColor) -> NSColor {
            var h: CGFloat = 0
            var s: CGFloat = 0
            var b: CGFloat = 0
            var a: CGFloat = 0
    ...
    public struct Grid {
        ...
        // #eeeeee
        public var color: NSColor = NSColor(red: 238/255.0, green: 238/255.0, blue: 238/255.0, alpha: 1)
    }
    
    public struct Axis {
        public var visible: Bool = true
        // #607d8b
        public var color: NSColor = NSColor(red: 96/255.0, green: 125/255.0, blue: 139/255.0, alpha: 1)
    ...
    public struct Dots {
        public var visible: Bool = true
        public var color: NSColor = NSColor.whiteColor()
    ...
    public var colors: [NSColor] = [
        NSColor(red: 0.121569, green: 0.466667, blue: 0.705882, alpha: 1),
        NSColor(red: 1, green: 0.498039, blue: 0.054902, alpha: 1),
        NSColor(red: 0.172549, green: 0.627451, blue: 0.172549, alpha: 1),
        NSColor(red: 0.839216, green: 0.152941, blue: 0.156863, alpha: 1),
        NSColor(red: 0.580392, green: 0.403922, blue: 0.741176, alpha: 1),
        NSColor(red: 0.54902, green: 0.337255, blue: 0.294118, alpha: 1),
        NSColor(red: 0.890196, green: 0.466667, blue: 0.760784, alpha: 1),
        NSColor(red: 0.498039, green: 0.498039, blue: 0.498039, alpha: 1),
        NSColor(red: 0.737255, green: 0.741176, blue: 0.133333, alpha: 1),
        NSColor(red: 0.0901961, green: 0.745098, blue: 0.811765, alpha: 1)
    ]
    
    override public init(frame: CGRect) {
        super.init(frame: frame)
//        self = NSColor.clearColor()
    }
    ...
    override public func drawRect(rect: CGRect) {
        ...
        if removeAll {
            let context = NSGraphicsContext.currentContext() //UIGraphicsGetCurrentContext()
            CGContextClearRect(context?.CGContext, rect) //CGContextClearRect(context, rect)
            return
        }
     ...
     * Handle touch events.
     */
    private func handleTouchEvents(touches: NSSet!, event: NSEvent) {
     ...
     */
    public func touchesEnded(touches: Set<NSTouch>, withEvent event: NSEvent?) {
     ...
     */
    public func touchesMoved(touches: Set<NSTouch>, withEvent event: NSEvent?) {
        handleTouchEvents(touches, event: event!)
    }
    ...
    private func drawAxes() {
        ...
        path.lineToPoint(CGPoint(x: width - x.axis.inset, y: y0))
        ...
        path.lineToPoint(CGPoint(x: x.axis.inset, y: y.axis.inset))
    ...
    private func drawLine(lineIndex: Int) {
        ...
        for index in 1..<data.count {
            ...
            path.lineToPoint(CGPoint(x: xValue, y: yValue))
            //path.addLineToPoint(CGPoint(x: xValue, y: yValue))
        }
        
        let layer = CAShapeLayer()
        layer.frame = self.bounds
        layer.path = path.CGPath(forceClose: true)
    ...
    private func drawAreaBeneathLineChart(lineIndex: Int) {
        ...
        path.lineToPoint(CGPoint(x: x.axis.inset, y: self.bounds.height - self.y.scale(data[0]) - y.axis.inset))
        ...
        for index in 1..<data.count {
            ...
            path.lineToPoint(CGPoint(x: x1, y: y1))
            //path.addLineToPoint(CGPoint(x: x1, y: y1))
        }
        // move down to x axis
        path.lineToPoint(CGPoint(x: self.x.scale(CGFloat(data.count - 1)) + x.axis.inset, y: self.bounds.height - self.y.scale(0) - y.axis.inset))
        ...
        // move to origin
        path.lineToPoint(CGPoint(x: x.axis.inset, y: self.bounds.height - self.y.scale(0) - y.axis.inset))
    ...
    private func drawXGrid() {
        ...
        for var i: CGFloat = start; i <= stop; i += step {
            ...
            path.lineToPoint(CGPoint(x: x1, y: y2))
    ...
    private func drawYGrid() {
        ...
        for var i: CGFloat = start; i <= stop; i += step {
            ...
            path.moveToPoint(CGPoint(x: x1, y: y1))
            path.lineToPoint(CGPoint(x: x2, y: y1))
    ...
    private func drawXLabels() {
        ...
        for (index, _) in xAxisData.enumerate() {
            ...
            let label = NSTextField(frame: CGRect(x: xValue, y: y, width: width, height: x.axis.inset))
//            label.font = NSFont.preferredFontForTextStyle()
//            label.textAlignment = .Center
    ...
    private func drawYLabels() {
        ...
        for var i: CGFloat = start; i <= stop; i += step {
            ...
            let label = NSTextField(frame: CGRect(x: 0, y: yValue, width: y.axis.inset, height: y.axis.inset))
            label.alignment = .Center
            label.stringValue = String(Int(round(i)))
...
class DotCALayer: CALayer {
    ...
    var dotInnerColor = NSColor.blackColor()
